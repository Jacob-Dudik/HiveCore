...
module instr_mem
  import core_pkg::*;
  #(
    parameter MEM_SIZE_BYTES = 1024 * 1024, // 1 MB to fit official RISC-V tests
  ...
  // Load firmware into memory at startup
  initial begin
  ...
  always_comb begin
    ...
    if (imem_req_valid_i) begin
      for (int i = 0; i < FETCH_WIDTH; i++) begin
        ...
        // Calculating address for the specific fetch slot, masking it to fit the memory size
        // This cleanly maps the 0x8000_0000 reset PC to physical memory address 0.
        logic [MEM_ADDR_WIDTH-1:0] addr = (imem_req_addr_i + (i * 4)) & (MEM_SIZE_BYTES - 1);
